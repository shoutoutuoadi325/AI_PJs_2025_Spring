...
for category, category_questions in questions_by_category.items():
    ...
    for question in category_questions:
        # split question into text, options, and correct answer
        turns = question["turns"]
        question_text = turns[0]
        options = [t for t in turns[1:] if not t.startswith("Correct Answer")]
        correct_answer = next((t.split("Correct Answer: ")[1] for t in turns if t.startswith("Correct Answer")), None)
        question_id = question["question_id"]
        
        # add question header (and options + answer for MCQs)
        with open("ans.md", 'a', encoding='utf-8') as f:
            if options:
        ...
        for model in models:
            # prepare user content: include options for MCQs, exclude the correct answer
            if options:
